...
export function validateFormData(form) {
    ...
    let response = false
    const keys = ['birthdate','cityBirth','cityLocation','connectivity','documentNumber','email','entityName','ethnicity','firstName','frontDocument','gender','identityDocument','lastName','neighborhood','phoneNumber','stateBirth','stateLocation','typeEntity','zona']
    keys.forEach((item) => {
        if (form[item] !== "" && form[item] !== null && form[item] !== undefined) response = true
    })
    return response
}
